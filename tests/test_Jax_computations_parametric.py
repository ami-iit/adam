...
def test_mass_matrix():
    ...
    mass_test_hardware = np.array(comp_w_hardware.mass_matrix(H_b,s_, original_length, original_density))
    assert mass_test - mass_test_hardware == pytest.approx(0.0, abs=1e-5)

def test_CMM():
    Jcm_test = comp.centroidal_momentum_matrix(H_b, s_)
    Jcm_test_hardware = np.array(comp_w_hardware.centroidal_momentum_matrix(H_b,s_, original_length, original_density))
    assert Jcm_test - Jcm_test_hardware == pytest.approx(0.0, abs=1e-5)

def test_CoM_pos():
    CoM_test = comp.CoM_position(H_b, s_)
    CoM_hardware  = np.array(comp_w_hardware.CoM_position(H_b,s_, original_length, original_density)) 
...
def test_jacobian():
    ...
    J_test_hardware = np.array(comp_w_hardware.jacobian("l_sole",H_b, s_, original_length, original_density))
    assert J_test - J_test_hardware == pytest.approx(0.0, abs=1e-5)

def test_jacobian_non_actuated():
    J_test = comp.jacobian("head", H_b, s_)
    J_test_hardware = np.array(comp_w_hardware.jacobian("head",H_b, s_, original_length, original_density))
    assert J_test - J_test_hardware == pytest.approx(0.0, abs=1e-5)

def test_fk():
    H_test = comp.forward_kinematics("l_sole", H_b, s_)
    H_with_hardware_test = np.array(comp_w_hardware.forward_kinematics("l_sole",H_b, s_, original_length, original_density))
...
def test_fk_non_actuated():
    ...
    H_with_hardware_test = np.array(comp_w_hardware.forward_kinematics("head",H_b, s_, original_length, original_density))
    assert H_with_hardware_test[:3,:3] - H_test[:3, :3] == pytest.approx(0.0, abs=1e-5)
    assert H_with_hardware_test[:3,3] - H_test[:3, 3] == pytest.approx(0.0, abs=1e-5)

def test_bias_force():
    h_test = comp.bias_force(H_b, s_, vb_, s_dot_)
    h_with_hardware_test = np.array(comp_w_hardware.bias_force(H_b, s_, vb_, s_dot_, original_length, original_density))
...
def test_coriolis_term():
    ...
    C_with_hardware_test = np.array(comp_w_hardware.coriolis_term(H_b, s_, vb_, s_dot_, original_length, original_density))
    assert C_with_hardware_test - C_test == pytest.approx(0.0, abs=1e-4)

def test_gravity_term():
    G_test = comp.gravity_term(H_b, s_)
    G_with_hardware_test = comp_w_hardware.gravity_term(H_b,s_, original_length, original_density)
