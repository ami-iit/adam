...
def setup_test(tests_setup) -> KinDynComputationsBatch | RobotCfg | State:
    ...
    n_samples = 10
    state.H = torch.tile(torch.tensor(state.H), (n_samples, 1, 1))
    state.joints_pos = torch.tile(torch.tensor(state.joints_pos), (n_samples, 1))
    state.base_vel = torch.tile(torch.tensor(state.base_vel), (n_samples, 1))
    state.joints_vel = torch.tile(torch.tensor(state.joints_vel), (n_samples, 1))
