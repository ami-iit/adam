...
comp = KinDynComputations.from_mujoco_xml(model_path_xml, joints_name_list)
...
def test_jacobian():
    ...
    kinDyn.getFrameFreeFloatingJacobian("l_hand", iDyntreeJ_)
    iDynNumpyJ_ = iDyntreeJ_.toNumPy()
    J_test = comp.jacobian("l_hand", H_b, joints_val)
...
def test_jacobian_dot():
    J_dot = comp.jacobian_dot("l_hand", H_b, joints_val, base_vel, joints_dot_val)
    Jdotnu = kinDyn.getFrameBiasAcc("l_hand")
